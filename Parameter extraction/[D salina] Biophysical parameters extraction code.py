...
output_filenames = ["D_salina.xlsx"]

PIXEL_SIZE = 0.75  # μm/px
...
for src_dir, output_filename in zip(src_dirs, output_filenames):
    data = pd.DataFrame(columns=[
        ...
        "Integrated Density", "Perimeter(μm)", "Equivalent Diameter(μm)", "Solidity", "Convexity", "Circularity",
        "Aspect Ratio", "Extent", "Rectangularity", "Deformation Index", "Intensity Std", "Intensity CoV"
    ])
    ...
    for i, src_filename in enumerate(os.listdir(src_dir)):
            ...
            for threshold in range(150, 155, 1):
                ...
                for contour in contours:
                    if contour.shape[0] >= 5:  # fitEllipse requires at least 5 points
                        ...
                        if 10 <= minor_axis <= 25 and (major_axis / minor_axis) < 1.5 and shape_factor > 0.5:
        ...
        if best_alpha2 is None or best_ellipse is None or best_contour is None:            
            continue
        ...
        if best_ellipse is not None:
            ...
            size_um2 = total_pixel_count * (PIXEL_SIZE ** 2)
            ...
            perimeter_um = contour_perimeter * PIXEL_SIZE
            equiv_diameter_um = (np.sqrt(4 * contour_area / np.pi) * PIXEL_SIZE) if contour_area > 0 else np.nan
