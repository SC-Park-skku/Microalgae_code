...
# 경로 설정
src_dirs = [r"C:\Python\workspace\Microalgae\final2\D_salina_5day_set2_3\crops\cell"]

output_filenames = ["D_salina_5day_set2_3.xlsx"]

# DataFrame을 결과에 저장하기 위해 생성
for src_dir, output_filename in zip(src_dirs, output_filenames):
    data = pd.DataFrame(columns=["Filename", "Major Axis", "Minor Axis", "Size(μm²)", "Eccentricity", "Ellipse Mean Gray Scale"])
    ...
    for i, src_filename in enumerate(os.listdir(src_dir)):
        ...
        for alpha2 in np.arange(1.0, 5.0, 0.1):
            for threshold in range(180, 185, 1):
                # Adjust contrast
        ...
        if best_ellipse is not None:
            ...
            # 타원 그린 이미지 저장 폴더 설정
            output_image_folder = r"C:\Python\workspace\Microalgae\final2\D_salina_5day_set2_3\crops\draw"
            ...
            # 최종 eccentricity와 size 기반으로 major/minor 재계산
