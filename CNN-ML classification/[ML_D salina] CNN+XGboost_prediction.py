...
# =========================
EXCEL_DIR = r"Path"
BASE_IMG_DIR = r"Dir path"

stage_map = {
    "D_salina_0day.xlsx": 0,
    "D_salina_1day.xlsx": 1,
    "D_salina_3day.xlsx": 3,
    "D_salina_5day.xlsx": 5,
    "D_salina_7day.xlsx": 7,
}

stage_to_subdir = {
    0: r"D_salina_0day\crops\cell",
    1: r"D_salina_1day\crops\cell",
    3: r"D_salina_3day\crops\cell",
    5: r"D_salina_5day\crops\cell",
    7: r"D_salina_7day\crops\cell",
}
...
CNN_CACHE = CACHE_DIR / "cnn_feats.npy"
...
for fn in os.listdir(EXCEL_DIR):
    if fn.endswith(".xlsx") and fn in stage_map:
        ...
        if "Filename" not in df.columns:
            raise KeyError("")
        dfs.append(df.sample(n=min(50000, len(df)), random_state=RANDOM_STATE))
...
need_cols = [
    ...
    "Betacarotene", "Integrated Density", "Perimeter(μm)",
...
X_tab = scaler.fit_transform(X_num.values)     
X_all = np.hstack([X_tab, X_cnn])            

print("🧱 Final feature shape (Tab+CNN):", X_all.shape)

SCALE_CNN_ONLY = True            
if SCALE_CNN_ONLY:
    cnn_scaler = StandardScaler()
    X_cnn_only = cnn_scaler.fit_transform(X_cnn)
else:
    cnn_scaler = None
    X_cnn_only = X_cnn
...
print("🎯 Test Accuracy (Tab+CNN):", accuracy_score(y_test, y_pred_tabcnn))
...
print("\n📊 Confusion Matrix (Tab+CNN):\n", cm)
print("\n🧾 Classification Report (Tab+CNN):\n",
...
def run_inference_one(excel_path: str, img_subdir: str, save_path: str):
    ...
    X_new_tab_scaled = scaler.transform(X_new_tab.values)
    ...
    pred_stage_value = le.inverse_transform(pred_idx)   # 0/1/3/5/7
    ...
    print("\n==== Hard Prediction % by stage (0,1,3,5,7) ====")
...
for cfg in BATCH_INFER_LIST:
    run_inference_one(cfg["excel_path"], cfg["img_subdir"], cfg["save_path"])
