...
BEST_PATH = MODEL_DIR / "dsalina_resnet18_best.pt"
...
le = LabelEncoder()
y = le.fit_transform(df_all["stage"].values)        # 0..C-1
class_values = le.inverse_transform(np.arange(len(le.classes_)))  # [0,1,3,5,7]
...
train_idx, val_idx = train_test_split(idx, test_size=TEST_SIZE, stratify=y, random_state=SEED)
...
class ImgClsDataset(Dataset):
    def __init__(self, df: pd.DataFrame, labels: np.ndarray, transform):
        self.df = df; self.labels = labels; self.t = transform
    def __len__(self): return len(self.df)
    def __getitem__(self, i):
        p = self.df.at[i, "img_path"]; y = self.labels[i]
        try:
